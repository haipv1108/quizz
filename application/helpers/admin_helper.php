...
//user
if(!function_exists('vali_add_user')){
	function vali_add_user(){
		$ci = &get_instance();
...
if(!function_exists('vali_edit_user')){
	function vali_edit_user(){
		$ci = &get_instance();
		$ci->form_validation->set_rules('username', 'User Name', 'required|min_length[5]|max_length[40]'); 
		$ci->form_validation->set_rules('email', 'Email', 'required|valid_email');
		$ci->form_validation->set_rules('gender', 'Gender', 'required');
...
if(!function_exists('get_info_add_user')){
	function get_info_add_user(){
		$ci = &get_instance();
		$json_string = array(
...
if(!function_exists('get_info_edit_user')){
	function get_info_edit_user(){
		$ci = &get_instance();
		$json_string = array(
							'birthday' => $ci->input->post('birthday'),
							'gender' => $ci->input->post('gender'),
